...
void init_game::targetSet(Vector2f const &size, Vector2f const &position, Color clr, RectangleShape& tar) {
    tar.setSize(size);
    tar.setPosition(position);
    tar.setFillColor(clr);
}
...
void init_game::map_objs_init() {
    ...
    targetSet(Vector2f(40.f, 40.f), Vector2f(50, 50), Color::Black, target[1]);
    targetSet(Vector2f(10.f, 10.f), Vector2f(20, 30), Color::Black, target[2]);
}
